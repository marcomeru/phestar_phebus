    ...
    for ii in range(dimet) : # for each time step
        ...
        for jj in range(nbst) : # for each selected star
            ...
            if D - 1.5*np.radians(max_sep) <= np.arccos(vzurf_star[1]) <= D + 1.5*np.radians(max_sep) :
                ...
                if 100 <= scanpos_min <= 3990 :
                    
                    if ettime[ii] >= mtm_separation or (ettime[ii] < mtm_separation and (scanpos_min <= 2100 or 2850 <= scanpos_min)) :
                ...
                if scanpos_min != scanpos_max :
                    ...
                    # Ceil position
                    if 100 <= scanpos_max <= 3990 :
                            
                        if ettime[ii] >= mtm_separation or (ettime[ii] < mtm_separation and (scanpos_max <= 2100 or 2850 <= scanpos_max)) :
